...
- evidence: owner screenshot refs/inbox/"lights or awning - unsure what this is.png"
  ...
  pennant string" in the 002 review; still unresolved.
- expected: awnings ATTACH to the facade (bracket or cantilever from the wall
  face), bulbs read as a lit row ON the awning edge (canvas texture or tiny
  instanced spheres flush to the fascia), continuous per storefront — no gaps
  mid-lot, no free-floating geometry.
- route: task 010 (bar likenesses) owns the hero-bar facades and rebuilds
  these fronts from photos — fix there, AFTER 009 moves the lots.
